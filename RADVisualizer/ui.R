  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem(
        ...
        radioButtons(
          "testFileOpt",
          "Upload or split train set:",
          choices = c("Upload", "Split"),
          inline = TRUE
        ),
        conditionalPanel(
          condition = "input.testFileOpt == 'Upload'",
          radioButtons(
            "delimTestFile",
            "Select file format:",
            choices = c("csv", "tsv"),
        ...
        conditionalPanel(
          condition = "input.testFileOpt == 'Split'",
          numericInput(
            "trainRatio",
            "Enter train size (0 - 1)",
            min = 0,
            max = 1.0,
            step = 0.05,
            value = 0.7
    ...
    tabItems(
      ...
      tabItem(tabName = "linearRegression",
              fluidRow(
                ...
                box(
                  "Predictions",
                  wellPanel(tableOutput("predictionsLR"),
      ...
      tabItem(tabName = "decisionTree",
              fluidRow(
                ...
                box(
                  "Predictions",
                  wellPanel(tableOutput("predictionsDT"),
                            style = "overflow-y:scroll; max-height: 200px;
                            overflow-x:scroll; auto")
                ),
